...
function TaskCreate({onCreate, taskProp, taskformUpdate, onUpdate}) {
    // input için state
    // içerisinde ki değerleri güncelleye basınca yazılan verileri göstermek için,
    const [title,setTitle] = useState(taskProp ? taskProp.title : "");
    // textarea için state
    const [taskDesc,setTaskDesc] = useState(taskProp ? taskProp.taskDesc : "");
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (taskformUpdate ) {
            onUpdate(taskProp.id,title,taskDesc);
        }else {
